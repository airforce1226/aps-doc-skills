...
1. Gather content (report type + reader executive/working-level); never invent unknowns → mark `(미정 — 추후 확정)`.
...
4. Verify the deck (slide count, `author == "IT전략팀"`; in native mode confirm text/tables/shapes are
...
- **Do not ask the security grade.** All APS internal reports are **대외비**, so apply the "대외비" badge to every slide by default. Change only when the user explicitly requests another grade/공개.
- **Always ask about cover/closing slides (when multi-slide).** If the deck is 2+ slides, **before** assembling deck.html, ask the user whether to include the cover (`01-cover.html`) / closing (`13-closing.html`) slides, and apply per their answer. Do not add or omit them arbitrarily (for a single-slide deck, do not ask).
